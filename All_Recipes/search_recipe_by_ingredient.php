...
<?php
    ...
    //we might might want to do something more like:::     where name like "%all-purpose flour%"
	$query = "SELECT count(*) as id_count, recipe_id from ingredients where match (name) against ($ingredient1 in boolean mode) or match (name) against ($ingredient2 in boolean mode) or match (name) against ($ingredient3 in boolean mode) or match (name) against ($ingredient4 in boolean mode) or match (name) against ($ingredient5 in boolean mode)  GROUP BY recipe_id ORDER BY id_count DESC LIMIT 10";	
	$recipes = mysqli_query($link, $query) or die('Query failed...: ' . mysql_error());


	
	$recipe_id = array();

	while ($recipe_array = mysqli_fetch_array($recipes, MYSQL_ASSOC)) {
		foreach ($recipe_array as $key => $value) {
			if ($key == "recipe_id") {
				array_push($recipe_id, $value);
			}
		}
	}
...
foreach ($recipe_id as $id) {
	...
	$query = 'SELECT * FROM recipes where is_deleted = 0 AND id='.$id;
